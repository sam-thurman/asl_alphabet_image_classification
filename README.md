...
OpenCV works the same way as any basic webcam, by recording frames every 20th or 30th (or whatever you specify) of a second and displaying them. These frames can be fed to the model to predict on, so all that needs to happen is the frames need to be formated in a way that is compatible with the model input.  This involves a simple resizing of the frame, after which it is converted to array format and fed to the model for predictions.  There are some slightly ugly steps after the class prediction is made (model output is an array of probabilities of the image belonging to each class 0-28), and the final letter prediction is returned and displayed on the frame being shown to the user.